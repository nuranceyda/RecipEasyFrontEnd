...
signupButton.addEventListener('click', async function (e) {
    ...
    if (creatable) {
        let result = await createUser(usernameSignup,passwordSignup,firstnameSignup,lastnameSignup);
...
export async function getUser(username) {
    ...
    const result = await axios({
        method: 'get',
        //url: link + `user/${name}`,
        url: `https://cors-anywhere.herokuapp.com/https://recipeasy426.herokuapp.com/user/${name}`
    });
